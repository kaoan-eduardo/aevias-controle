  ...
  return (
    <div className="relatorio-page bg-white min-h-screen">
      ...
      <div className="w-full max-w-[210mm] mx-auto bg-white shadow-xl print:shadow-none p-4 print:p-2 flex flex-col gap-0">
        ...
        <div className="border border-slate-400 p-4 text-[13px] mb-0">
        ...
        <table className="w-full border-collapse border border-slate-400 text-[12px] mb-0">
          <tbody>
            <tr>
              <td className="border border-slate-400 px-3 py-2 font-semibold w-[14%]">ESTRUTURA:</td>
              <td className="border border-slate-400 px-3 py-2 w-[35%]">{ensaio.estrutura || ''}</td>
              <td className="border border-slate-400 px-3 py-2 font-semibold w-[15%]">CONSTRUTORA:</td>
              <td className="border border-slate-400 px-3 py-2">{ensaio.construtora || ''}</td>
            </tr>
            <tr>
              <td className="border border-slate-400 px-3 py-2 font-semibold">NOTA FISCAL</td>
              <td className="border border-slate-400 px-3 py-2">{ensaio.nota_fiscal || ''}</td>
              <td className="border border-slate-400 px-3 py-2 font-semibold">ESTACA DE MOLDAGEM:</td>
              <td className="border border-slate-400 px-3 py-2">{ensaio.estaca_moldagem || ''}</td>
            </tr>
          </tbody>
        </table>
        ...
        <table className="w-full border-collapse border border-slate-400 text-[12px] mb-0">
          <tbody>
            <tr>
              <td className="border border-gray-400 px-3 py-2.5 w-[50%]">
                <span className="font-semibold">SLUMP TEST (mm):</span>
                <span className="ml-2">{ensaio.slump_test ?? ''}</span>
              </td>
              <td className="border border-gray-400 px-3 py-2.5">
   ...
   return (
     <table className="w-full border-collapse border border-slate-400 text-[12px]">
      <tbody>
        ...
        <tr>
          <td className="border border-slate-400 px-3 py-2 font-semibold w-[28%]">N° CORPOS DE PROVA MOLDADOS:</td>
           <td className="border border-slate-400 px-3 py-2 text-center font-bold" colSpan={2}>{totalCPs || ''}</td>
           <td className="border border-slate-400 px-3 py-2 font-semibold text-center">DIMENSÕES:</td>
           <td className="border border-slate-400 px-3 py-2 text-center font-bold text-blue-800" colSpan={series.length * 2 || 1}>
        ...
        <tr className="bg-slate-100">
          ...
          {series.length === 0 && <td className="border border-slate-400 px-3 py-2" colSpan={4}></td>}
          </tr>
          {/* Linha: N° CP */}
          <tr>
            <td className="border border-slate-400 px-3 py-2 font-semibold">N° CP</td>
            <td className="border border-slate-400 px-3 py-2 text-center text-gray-500 italic"></td>
            <td className="border border-slate-400 px-3 py-2 text-center"></td>
            {series.map((s, si) =>
              s.map((cp, ci) => (
                <td key={`${si}-${ci}`} className="border border-slate-400 px-3 py-2 text-center font-semibold">{cp.numero_cp || ''}</td>
            ))
          )}
          {series.length === 0 && [0,1,2,3].map(i => <td key={i} className="border border-slate-400 px-3 py-2"></td>)}
          </tr>
          {/* DATA DA RUPTURA */}
          <tr>
            <td className="border border-slate-400 px-3 py-2 font-semibold">DATA DA RUPTURA</td>
            <td className="border border-slate-400 px-3 py-2 text-center text-gray-400">-</td>
            <td className="border border-slate-400 px-3 py-2 text-center text-gray-400">-</td>
            {series.map((s, si) =>
              s.map((cp, ci) => (
                <td key={`${si}-${ci}`} className="border border-slate-400 px-3 py-2 text-center">{cp.data_ruptura ? fmtDate(cp.data_ruptura) : ''}</td>
            ))
          )}
          {series.length === 0 && [0,1,2,3].map(i => <td key={i} className="border border-slate-400 px-3 py-2"></td>)}
          </tr>
          {/* CARGA DE RUPTURA */}
          <tr>
            <td className="border border-slate-400 px-3 py-2 font-semibold">CARGA DE RUPTURA</td>
            <td className="border border-slate-400 px-3 py-2 text-center text-gray-600 italic">C</td>
            <td className="border border-slate-400 px-3 py-2 text-center">tf</td>
            {series.map((s, si) =>
              s.map((cp, ci) => (
                <td key={`${si}-${ci}`} className="border border-slate-400 px-3 py-2 text-center">{fmtN(cp.carga_ruptura, 2)}</td>
            ))
          )}
          {series.length === 0 && [0,1,2,3].map(i => <td key={i} className="border border-slate-400 px-3 py-2"></td>)}
          </tr>
          {/* ÁREA DO CP */}
          <tr>
            <td className="border border-slate-400 px-3 py-2 font-semibold">ÁREA DO CORPO DE PROVA</td>
            <td className="border border-slate-400 px-3 py-2 text-center text-gray-600 italic">A</td>
            <td className="border border-slate-400 px-3 py-2 text-center">cm²</td>
            {series.map((s, si) =>
              s.map((cp, ci) => (
                <td key={`${si}-${ci}`} className="border border-slate-400 px-3 py-2 text-center">{fmtN(cp.area_cp, 2)}</td>
            ))
          )}
          {series.length === 0 && [0,1,2,3].map(i => <td key={i} className="border border-slate-400 px-3 py-2"></td>)}
          </tr>
          ...
          <tr className="bg-slate-50">
            <td className="border border-slate-400 px-3 py-2 font-semibold">RESISTÊNCIA</td>
            <td className="border border-slate-400 px-3 py-2 text-center text-[9px] text-gray-600 italic">
              Rfck = C/A × 1000/10,197
            </td>
            <td className="border border-slate-400 px-3 py-2 text-center">MPa</td>
            {series.map((s, si) =>
              s.map((cp, ci) => (
                <td key={`${si}-${ci}`} className="border border-slate-400 px-3 py-2 text-center text-blue-800 font-semibold">{fmtN(cp.resistencia, 2)}</td>
            ))
          )}
          {series.length === 0 && [0,1,2,3].map(i => <td key={i} className="border border-slate-400 px-3 py-2"></td>)}
          </tr>
          ...
          <tr className="bg-slate-100">
            <td className="border border-slate-400 px-3 py-2 font-semibold">RESIST. DO EXEMPLAR</td>
            <td className="border border-slate-400 px-3 py-2 text-center text-gray-600 italic">Rfck</td>
            <td className="border border-slate-400 px-3 py-2 text-center">MPa</td>
            {series.map((s, si) => (
              <td key={si} className="border border-slate-400 px-3 py-2 text-center font-bold text-blue-900" colSpan={2}>
   ...
   return (
     <table className="w-full border-collapse border border-slate-400 text-[12px]">
      <tbody>
        {/* N° CPs */}
        <tr>
          <td className="border border-slate-400 px-3 py-2 font-semibold w-[28%]">N° CORPOS DE PROVA EXTRAÍDOS:</td>
          <td className="border border-slate-400 px-3 py-2 text-center font-bold" colSpan={2}>{totalCPs || ''}</td>
          <td className="border border-slate-400 px-3 py-2 font-semibold text-center">DIMENSÕES:</td>
          <td className="border border-slate-400 px-3 py-2 text-center font-bold" colSpan={series.length * 2 || 1}>
            CP PRISMÁTICO
          </td>
          <td className="border border-slate-400 px-3 py-2 text-[9px] text-gray-600 text-center">h = 100 cm</td>
          <td className="border border-slate-400 px-3 py-2 text-[9px] text-gray-600 text-center">g = 9,80665 m/s²</td>
        </tr>
        {/* IDADE */}
        <tr className="bg-slate-100">
          <td className="border border-slate-400 px-2 py-1 font-semibold">IDADE</td>
        ...
        <tr className="bg-slate-100">
          <td className="border border-slate-400 px-3 py-2 font-semibold">RESIST. DO EXEMPLAR</td>
          <td className="border border-slate-400 px-3 py-2 text-center text-gray-600 italic">Rfctm</td>
          <td className="border border-slate-400 px-3 py-2 text-center">MPa</td>
          {series.map((s, si) => (
            <td key={si} className="border border-slate-400 px-3 py-2 text-center font-bold text-blue-900" colSpan={2}>
              {resistenciaExemplar(s)}
