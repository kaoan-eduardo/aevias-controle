...
export default function RelatorioChecklistMRAF({ checklist, obra, regional, project }) {
  ...
  const photoPages = checklist.fotos && checklist.fotos.length > 0 
    ? Array.from({ length: Math.ceil(checklist.fotos.length / photosPerPage) }, (_, i) =>
        checklist.fotos.slice(i * photosPerPage, (i + 1) * photosPerPage)
      )
    : [];
